...
def upload_attachment_from_bytes(data: bytes, dest_blob_dir_name: str, dest_blob_name: str) -> str:
    ...
    dest_blob_path = os.path.join(
        'attachment', dest_blob_dir_name, dest_blob_name)
...
def upload_image_from_bytes(data: bytes, dest_blob_name: str) -> str:
    """
    Uploads an image file to the bucket.
    Returns the public url of the image file.
    """
    bucket_name = os.environ['GCS_BUCKET']
